...
class CoordinateManager:
    r"""
    坐标管理器主要是通过建立 D 维坐标值与储存索引的映射来用来管理坐标。
    """
    def __init__(
        self,
        D: int = 0,
        ...
        # pybind/extern.hpp:instantiate_manager 对 Python 端暴露 CoordinateMapManager，这里跟据获取 CPU 或 GPU 进行获取
        self._CoordinateManagerClass = getattr(_C, "CoordinateMapManager" + postfix)
        self._manager = self._CoordinateManagerClass(
            minkowski_algorithm, num_threads
        )  # 实例化后端实现类
