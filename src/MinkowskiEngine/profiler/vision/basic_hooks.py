...
def count_me_conv(m, x, y):
    x=x[0]
    aux_x=ME.SparseTensor(
        features=torch.ones([x.F.shape[0],1]),
        coordinate_manager=x.coordinate_manager,
        coordinate_map_key=x.coordinate_map_key,
        device=x.device
    )
    SumPooling=ME.MinkowskiSumPooling(kernel_size=-1,kernel_generator=m.kernel_generator, dimension=m.dimension).to(x.device)
    n_activate_kenel_volumn=torch.sum(SumPooling(aux_x).F)
    N_in, C_in=x.shape
    N_out, C_out=y.shape
    m.total_ops+=C_in*C_out*n_activate_kenel_volumn.cpu().item()
